...
class CategorySelectApp:
    def __init__(self, master, character_challenge=False, username=None):
        ...
        tk.Label(master, text="Welcome to the Coding Quiz!", font=("Helvetica", 18, "bold"), bg="#f0f4f8", fg="#333").pack(pady=5)
        tk.Label(master, text="Enter your name:", font=("Helvetica", 12), bg="#f0f4f8", fg="#333").pack()
        self.name_entry = tk.Entry(master, font=("Helvetica", 12), justify="center")
        ...
        tk.Label(master, text="Spin the wheel to choose a category:", font=("Helvetica", 14), bg="#f0f4f8", fg="#333").pack(pady=1)

        self.categories = ["JavaScript", "HTML", "Java", "C++", "SQL", "Python"]

        self.image_refs = []  # ✅ Prevent icon images from being garbage collected

        def load_icon(path):
            icon = PhotoImage(file=path).subsample(16, 16)
            self.image_refs.append(icon)  # ✅ Persist image reference
            return icon       
        ...
        # Force fresh wheel image regeneration
        self.master.after(100, self.build_wheel)
        ...
        self.leaderboard_button = tk.Button(
            ...
            font=("Helvetica", 12, "bold"),     # larger, bolder font
            height=2,                           # taller button
            width=20,                           # optional fixed width
            bg="#4CAF50",
            fg="white",
            command=self.show_leaderboard
        )
        self.leaderboard_button.pack(pady=10)  # more vertical space around it        
    ...
    def show_leaderboard(self):
        ...
        tk.Label(leaderboard_win, text="Top Players", font=("Helvetica", 14, "bold"), bg="#f0f4f8").pack(pady=5)
    ...
    def launch_character_quiz(self, name, category):
        ...
        new_window.after(0, lambda: init_quiz())  # This defers execution properly
      # Let Tkinter fully initialize first
    ...
    def build_wheel(self):
            global angle
            self.wheel = SpinningWheel(self.master, self.categories, self.icons, self.start_quiz, initial_angle=angle)
            self.wheel.pack(pady=10)     
...
class CharacterChallengeQuiz:
    ...
    def __init__(self, master, username, category):
        ...
        # Hold image references to prevent garbage collection
        self.image_refs = []

        # Delay starting the thread until Tkinter loop is running
        self.master.after_idle(self.start_question_thread)
    ...
    def start_question_thread(self):
        def thread_target():
            ...
            try:
                self.master.after(100, lambda: self.process_questions(questions))
            except RuntimeError as e:
                print("[ERROR] Failed to update UI from thread:", e)
    ...
    def get_questions(self):
        ...
        try:
            with open("questions.json", "r") as f:
                all_questions = json.load(f)
            questions = [q for q in all_questions if q['topic'].lower() == self.category.lower()]
    ...
    def process_questions(self, questions):
        if not questions:
            messagebox.showerror("Error", "AI and local question loading failed.")
            self.master.destroy()
            return
        ...
        if USE_AI_QUESTIONS:
            ...
            def update_header():
                try:
                    ...
                    print("[WARN] Could not update header:", e)

            self.master.after(100, update_header)
            ...
            def fallback_to_local():
                try:
                    ...
                    def show_error():
                        messagebox.showerror("Error", f"AI and local question loading failed:\n{e}")
                        self.master.destroy()
                    self.master.after(100, show_error)

            if new_questions:
                self.master.after(100, on_success)
            else:
                print("[WARN] Could not get unique AI question. Falling back to local.")
                self.master.after(100, fallback_to_local)         
